...
func CreateServices(c *fiber.Ctx) error {
	...
	if result := database.Database.Db.Where("name=?", request.Name).First(&existingservices); result.RowsAffected != 0 {
		return c.Status(fiber.StatusBadRequest).JSON(fiber.Map{
			"success": false,
	...
	if err := database.Database.Db.Create(&services).Error; err != nil {
...
func GetAllServices(c *fiber.Ctx) error {
	...
	if err := database.Database.Db.Find(&servicess).Error; err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"success": false,
...
func GetServiceById(c *fiber.Ctx) error {
	...
	if err := database.Database.Db.First(&services, id).Error; err != nil {
...
func UpdateServices(c *fiber.Ctx) error {
	...
	if err := database.Database.Db.Save(&services).Error; err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
...
func DeleteServices(c *fiber.Ctx) error {
	...
	if err := database.Database.Db.Delete(&services).Error; err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
