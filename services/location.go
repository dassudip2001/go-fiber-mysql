...
func CreateLocation(c *fiber.Ctx) error {
	...
	result := database.Database.Db.Where("name = ?", request.Name).First(&existingLocation)
	if result.Error == nil {
	...
	// Create the location
	if err := database.Database.Db.Create(&location).Error; err != nil {
...
func GetLocations(c *fiber.Ctx) error {
	...
	database.Database.Db.Preload("Children").Find(&locations)
...
func GetLocationById(c *fiber.Ctx) error {
	...
	if database.Database.Db.Preload("Children").First(&location, id).Error != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
	...
	if err := database.Database.Db.Preload("Children").Find(&location, id).Error; err != nil {
...
func UpdateLocation(c *fiber.Ctx) error {
	...
	if database.Database.Db.First(&location, id).Error != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
	...
	if err := database.Database.Db.First(&location, id).Error; err != nil {
		if err == gorm.ErrRecordNotFound {
			return c.Status(fiber.StatusNotFound).JSON(fiber.Map{
	...
	if err := database.Database.Db.Save(&location).Error; err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
			"error":   "failed to update location",
			"success": false,
		})
...
func DeleteLocation(c *fiber.Ctx) error {
	...
	if err := database.Database.Db.Delete(&location).Error; err != nil {
		return c.Status(fiber.StatusInternalServerError).JSON(fiber.Map{
